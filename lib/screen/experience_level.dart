  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: ColorUtils.kBlack,
        title:
            Text('Experience Level', style: FontTextStyle.kWhite16BoldRoboto),
        centerTitle: true,
        actions: [
          GetBuilder<ExperienceViewModel>(builder: (controller) {
            if (controller.apiResponse.status == Status.COMPLETE) {
              ExperienceLevelResModel response = controller.apiResponse.data;
            ...
            GetBuilder<ExperienceViewModel>(
              builder: (controller) {
                if (controller.apiResponse.status == Status.LOADING) {
                  return Padding(
                    padding: EdgeInsets.symmetric(vertical: Get.height * .32),
                    child: Center(
                          ...
                          children: [
                            GestureDetector(
                              onTap: () {
                                setState(() {
                                  selectedIndex = index;
                                  selectedLevel = response.data![index].slug;
                                });
                              },
                              child: Container(
                        ...
                        onTap: () {
                          ...
                          if (response.success == true) {
                            Get.to(PrimaryGoalsScreen(
                              ...
                              weight: widget.weight,
                              dob: widget.dob,
                              experienceLevel: selectedLevel ?? 'beginner',
                            ));
                          } else {
